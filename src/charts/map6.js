...
const stationGeometry = {
  1: { zone: "6_final", x: 16.9, y: 8.9, width: 8.1, height: 5.35 },
  2: { zone: "6_final", x: 25.1, y: 8.9, width: 8.1, height: 5.35 },
};
...
function drawHeatmap(data, metric) {
  ...
  const container = d3.select("#map-6");
  ...
  svg
    .append("image")
    .on("error", function () {
      console.error("Missing background image: 6_final.svg");
      d3.select(this).remove();
    })
    .attr("href", "6_final.svg")
    .attr("x", 0)
    .attr("y", 0)
    .attr("width", 1440)
    .attr("height", 974);
  ...
  svg
    ...
    .text(`Zone 6 (Final): ${metricFullName} Levels`);
