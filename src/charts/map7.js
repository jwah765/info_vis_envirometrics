...
const stationGeometry = {
  1: { zone: "7_inspect", x: 16.9, y: 8.9, width: 8.1, height: 5.35 },
  2: { zone: "7_inspect", x: 25.1, y: 8.9, width: 8.1, height: 5.35 },
};

// Scales setup
const xscale = d3.scaleLinear().domain([0, 50]).range([0, 1440]);
const yscale = d3.scaleLinear().domain([0, 33.79]).range([0, 974]); // Inverted for SVG coordinate system
...
async function loadAndProcessData() {
  try {
    ...
    console.log("Stations found:", [
      ...new Set(rawData.map((d) => d.station_no)),
    ]);
    updateVisualization("pm2_5");
  } catch (error) {
  ...
  console.log("Data sample check:", {
    stationType: typeof rawData[0].station_no,
    geometryKeys: Object.keys(stationGeometry),
    matchedStation: stationGeometry[rawData[0].station_no],
  });
}

function rowConverter(d) {
  sessionStorage.setItem("zoneStation", `${d.zone_id}_${d.station_no.trim()}`);

  return {
    date: new Date(d.datetime),
    zone_id: d.zone_id,
    station_no: d.station_no.trim(),
...
function updateVisualization(metric = "rh") {
  const aggregatedData = Array.from(
    ...
    (d) =>
      ...
      !isNaN(d.value) &&
      d.x !== undefined &&
      stationGeometry[d.station] // Ensure geometry exists
  );

  console.log("Visualization data:", aggregatedData);
  window.heatmapData = aggregatedData;
  drawHeatmap(window.heatmapData, metric);
}

function drawHeatmap(data, metric) {
  console.log("Drawing heatmap with", data.length, "stations");
  const container = d3.select("#map-7");
  ...
  svg
    .append("image")
    .on("error", function () {
      console.error("Missing background image: 7_inspect.svg");
      d3.select(this).remove();
    })
    .attr("href", "7_inspect.svg")
    .attr("x", 0)
    .attr("y", 0)
    .attr("width", 1440)
    .attr("height", 974);
  ...
  svg
    ...
    .text(`Zone 7 (Inspect): ${metricFullName} Levels`);
  ...
  // Rectangles with correct positioning
  svg
    .selectAll("rect")
    .data(data)
    .enter()
    ...
    // Keep existing positioning (original answer's suggestion was incorrect)
    .attr("x", (d) => xscale(d.x))
    ...
    .on("click", function (event, d) {
      ...
      const compositeId = `${d.zone}_${d.station}`;
      sessionStorage.setItem("zoneStation", compositeId);
      // Load line chart module ONLY when station is clicked
      import("./linechart.js").then((module) => {
        module.init();
      });
      // Trigger line chart update
      document.dispatchEvent(new CustomEvent("zoneStationChanged"));
      // Visual feedback
      d3.selectAll(".heatmap-cell").attr("stroke-width", 0);
  ...
  const labelGroups = svg
    ...
    .attr("transform", (d) => {
      ...
      const verticalOffset = 195; // Adjust this value to move the group up
  ...
  console.log(`Legend range for ${metric}: ${legendMin} to ${legendMax}`);
  console.log(`Color scale for ${metric}:`, colorScale);
  console.log(`Data for ${metric}:`, data);
  // Update legend
...
function addLegend(svg, colorScale, legendMin, legendMax, metric) {
  ...
  const legend = svg
    .append("g")
    .attr("class", "legend")
    .attr("transform", "translate(1295, 100)")
...
function setupEventListeners() {
  // Replace dropdown event listener with toggle button listener
...
export async function init() {
  ...
  try {
    ...
    // Show line chart container on initialization
